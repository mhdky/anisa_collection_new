    ...
    <div class="w-full py-4 px-5 flex justify-between items-center lg-1100:w-[1100px]">
        ...
        <div class="flex items-center">
            ...
            <ul class="flex ml-5 mt-1 md-950:ml-9">
                <li class="search-desktop mr-3 relative">
                    {{-- search desktop --}}
                    <i class="icon-search fas fa-search text-[0.75rem] cursor-pointer"></i>
                    <form action="/" method="get" autocomplete="off" class="form-search-desktop hidden">
                        <div class="bg-black-primary w-72 h-10 absolute z-30 top-8 -left-5 rounded-md">
                            <div class="relative w-full h-full p-[6px] flex items-center">
                                <i class="fas fa-sort-up text-black-primary absolute -top-[5px] left-[22px]"></i>
                                <input type="search" name="search" placeholder="Masukan keyword pencarian" id="input-search" class="bg-white text-sm w-full h-full mr-2 rounded-sm border border-white outline-none focus:ring-transparent focus:border-transparent">
                                <button type="submit" class="bg-yellow-primary h-full rounded-sm px-4"><i class="fas fa-search text-white text-sm"></i></button>
                            </div>
                        </div>
                    </form>
                </li>
        ...
        @auth
            <div class="flex items-center">
                ...
                {{-- nama pengguna --}}
                <div class="nama-pengguna bg-black-primary w-32 ml-5 py-[5px] px-2 flex justify-between items-center rounded-md relative cursor-pointer hover:bg-black-hover md-950:py-2">
                    ...
                    {{-- profil dan logout --}}
                    <div class="profile-logout bg-black-primary hidden w-32 h-20 absolute z-40 -bottom-[90px] left-[1px] rounded-md">
                        <div class="relative w-full h-full">
                            <i class="fas fa-sort-up absolute -top-[5px] left-1/2 -translate-x-1/2 text-black-primary"></i>
                            
                            {{-- profile --}}
                            <a href="/profile" class="w-full h-1/2 rounded-t-md px-2 flex items-center hover:bg-black-hover">
                                <i class="fas fa-user text-white text-[0.9rem] mr-2"></i>
                                <p class="text-white text-[0.8rem]">My profile</p>
                            </a>

                            <form method="POST" action="{{ route('logout') }}" class="w-full h-1/2 rounded-b-md hover:bg-black-hover">
                                @csrf
                                <button type="submit" class="w-full h-full rounded-b-md flex items-center hover:bg-black-hover px-2">
...
{{-- close form search --}}
<div class="close-form-search fixed z-20 top-0 right-0 bottom-0 left-0 hidden"></div>
